...
(function(_, $) {
	...
	VirtualList.prototype = {
		...
		prependElement: function($element) {
			this._prependedElementsBuffer.prepend($element.get(0));
		...
		appendElement: function($element) {
			this._appendedElementsBuffer.append($element.get(0));
		...
		_loadInitialElements: function($initialElement) {
			...
			for (i = 0; i < 50 && $currentElement; i++) {
				elementsBuffer.prepend($currentElement.get(0));
			...
			for (i = 0; i < 50 && $currentElement; i++) {
				elementsBuffer.append($currentElement.get(0));
		...
		_loadPreviousPendingElements: function(numberOfElementsToLoad) {
			...
			for (i = 0; i < numberOfElementsToLoad && $currentElement; i++) {
				prependedElementsBuffer.prepend($currentElement.get(0));
		...
		_loadNextPendingElements: function(numberOfElementsToLoad) {
			...
			var i;
			for (i = 0; i < numberOfElementsToLoad && $currentElement; i++) {
				appendedElementsBuffer.append($currentElement.get(0));
